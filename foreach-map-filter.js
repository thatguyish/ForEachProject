...
function showFirstAndLast(arr,i){
    const firstAndLast = arr.map((v)=>{
        const letters = `${v[0]}${v[v.length-1]}`;
        return letters;
    });
    return firstAndLast;
}
...
function vowelCount(str){
   ...
   stringAsArray.forEach(v=>{
    if(v=='a'||v=='e'||v=='i'||v=='o'||v=='u'||v=='A'||v=='E'||v=='I'||v=='O'||v=='U'){
        if(typeof(vowels[v])!='number'){
            vowels[v]=0;
        }
        vowels[v.toLowerCase()]+=1;
    }
   })
   return vowels;
}
...
function removeVowels(str) {
    ...
    let vowlessStr = strAsArray.filter(v=>{
        if(v!='a'&&v!='e'&&v!='i'&&v!='o'&&v!='u'&&v!='A'&&v!='E'&&v!='I'&&v!='O'&&v!='U'){
            return v;
        }
    }).map((v=>{
    ...
    if(vowlessStr.length==0){
        return [];
    }
    else{
        return vowlessStr.join('');
    }
...
function doubleOddNumbers(arr) {
    const doubledOddnums = arr.filter(v=>{
        if(v%2!=0){
            return true;
